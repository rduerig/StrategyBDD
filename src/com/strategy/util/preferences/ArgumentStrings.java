package com.strategy.util.preferences;

/**
 * @author Ralph Dürig
 */
public class ArgumentStrings {

	protected static final String PAR_GENERATE_FILES = "-f";
	protected static final String PAR_AVOID_FILES = "-nf";
	protected static final String PAR_MODE = "-gtp";
	protected static final String PAR_BOARD_SIZE = "-s";
	protected static final String PAR_TURNS = "-hgf";
	protected static final String PAR_TURNS_STRING = "-m";
	protected static final String PAR_OUT = "-out";
	protected static final String PAR_ALG = "-alg";

	protected static final String PAR_HELP = "-h";

}
